...
- 👤 **חשבון רכזת** - הרשמה ייעודית לרכזות (ללא צורך במנוי)
- ⭐ **תוכניות שמורות** - שמירת תוכניות מהאלפון בלחיצת כוכב
- 📝 **הערות אישיות** - הערה חופשית על כל תוכנית שמורה ("דיברתי איתה, פנויה רק בחנוכה")
- 🏷️ **מעקב סטטוס** - סימון כל תוכנית: שמורה / יצרתי קשר / סגרתי / לא רלוונטי
- 💰 **חישוב תקציב** - סיכום אוטומטי של עלויות התוכניות שנבחרו לאירוע

### 📩 מערכת פניות

- 📨 **פנייה דרך האתר** - רכזת שולחת פנייה למפיקה ישירות מכרטיס התוכנית
- 📧 **התראה במייל** - המפיקה מקבלת מייל מעוצב עם פרטי הפנייה
- 📥 **ניהול פניות** - עמוד "הפניות שלי" למפיקה עם סינון וסימון סטטוס (חדשה / נקראה / טופלה)

### 📊 סטטיסטיקות למפיקה

- 👁️ **מונה צפיות** - כמה פעמים נצפתה כל תוכנית באלפון
- 📩 **מונה פניות** - כמה פניות התקבלו לכל תוכנית
- ⭐ **מונה שמירות** - כמה רכזות שמרו כל תוכנית לאזור האישי שלהן

> ⚠️ **הערת שדרוג:** במעבר משדה `isAdmin` לשדה `role` יש להריץ פעם אחת את סקריפט המיגרציה על מסד הנתונים **לפני או מיד עם הפריסה**, אחרת המנהלת הקיימת תאבד גישה לפאנל הניהול:
> ```bash
